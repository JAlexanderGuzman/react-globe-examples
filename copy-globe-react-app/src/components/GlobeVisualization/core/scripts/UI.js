function UI() {
  ...
  for (var i = 0; i < buttons.length; i++) {
    ...
    newDiv +=
      "<td><div class='modeSelector' style='-webkit-transform: rotateY(" +
      angle +
...
UI.prototype.buildCategories = function (categories) {
  ...
  $.each(categories, function (i, val) {
    ...
    rgba =
      "rgba(" +
      Math.round(color.r * 295) +
      "," +
      Math.round(color.g * 295) +
      "," +
      Math.round(color.b * 295) +
      ",0.8)";
    catHTML +=
      "<td class='categoryButton' style='border-top:8px solid " +
      rgba +
      " ;'><div id=cat" +
      val.id +
...
UI.prototype.createProductBox = function (products) {
  var html =
    '<select class="productSelection"><option value="null" selected="selected">Select a product</option>';
...
UI.prototype.createSelectionBox = function (countries) {
  var html =
    '<select class="countrySelection"><option value="null" selected="selected">Select a country</option>';
