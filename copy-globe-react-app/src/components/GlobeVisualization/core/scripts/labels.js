...
LabelManager.prototype.toScreenXY = function (id, country, particleSystem) {
  var positions = window.geometry.attributes.position.array;
  ...
  $(ID).css({
    ...
    left:
      (p.x * window.innerWidth) / 2 + window.innerWidth / 2 - labelWidth / 2,
  });
};
...
LabelManager.prototype.toScreenXYZ = function (id, country, geometry) {
  ...
  if (
    cameraRay >
    Math.sqrt(
      Math.pow(camera.position.x - x, 2) +
        Math.pow(camera.position.y - y, 2) +
        Math.pow(camera.position.z - z, 2)
    ) +
      this.globeSize / 4
  ) {
    ...
    $(ID).css({
      top: Math.round(
        (-p.y * window.innerHeight) / 2 + window.innerHeight / 2 - 3
      ),
      left: Math.round(
        (p.x * window.innerWidth) / 2 + window.innerWidth / 2 - labelWidth / 2
      ),
...
LabelManager.prototype.animateLabels = function (
  countries,
  geometry,
  currentSetup,
  particleSystem
