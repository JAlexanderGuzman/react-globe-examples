function ParticleLinks(count, clock) {
  ...
  var uniforms = {
    ...
    texture: {
      type: "t",
      value: THREE.ImageUtils.loadTexture("images/dot.png"),
    },
  };
  ...
  this.linkGeometry.addAttribute(
    "position",
...
ParticleLinks.prototype.assignPositions = function (vertices, j, value) {
  ...
  for (var i = 0; i < offset; i++) {
    ...
    positions[this.totalAdded * 3 + i * 3] =
      vertices[indice].x +
      (vertices[indice + 1].x - vertices[indice].x) * average;
    positions[this.totalAdded * 3 + i * 3 + 1] =
      vertices[indice].y +
      (vertices[indice + 1].y - vertices[indice].y) * average;
    positions[this.totalAdded * 3 + i * 3 + 2] =
      vertices[indice].z +
      (vertices[indice + 1].z - vertices[indice].z) * average;
  }
  if (j === 9) {
